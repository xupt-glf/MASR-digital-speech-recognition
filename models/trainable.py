...
class TrainableModel(MASRModel):
    ...
    def fit(
        ...
        train_loader_shuffle = data.MASRDataLoader(
            train_dataset, train_batch_size, shuffle=True, num_workers=8
        )
        if sorta_grad:
            train_loader_sort = data.MASRDataLoader(
                train_dataset, train_batch_size, shuffle=False, num_workers=8
            )
        ...
        for epoch in range(epochs):
            ...
            for step, data_input in enumerate(train_loader):

                # print(step, inputs.shape, len(targets))
                x, x_lens = data_input[0],data_input[2]
                targets = data_input[1]
                x = x.to("cuda")
                gstep += 1
                outputs = self.forward(x, x_lens)
                print(outputs)
